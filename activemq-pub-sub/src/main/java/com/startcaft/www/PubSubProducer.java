...
/**
 * Pub/Sub 发布订阅模式下的消息发布者
 * Session创建的不是Queue而是Topic了
 */
    ...
    public static void main(String[] args) {
        {
            ...
            System.out.println("默认ActiveMQ连接用户名:" + PubSubProducer.USERNAME);
            System.out.println("默认ActiveMQ连接密码:" + PubSubProducer.PASSWORD);
            System.out.println("默认ActiveMQ连接地址:" + PubSubProducer.BROKEURL);

            try {
                //实例化连接工厂对象
                factory = new ActiveMQConnectionFactory(PubSubProducer.USERNAME,PubSubProducer.PASSWORD,PubSubProducer.BROKEURL);
                //通过连接工厂获取到连接对象，并启动
                ...
                //发送消息
                PubSubProducer.sendMessage(session,messageProducer);
